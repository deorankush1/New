 ...
 */
class PayUbiz_PayUbiz_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getStandardCheckoutFormFields()
    {
       ...
       $merchant_key = $this->getCurrencyMerchantKey();   
       $salt = $this->getCurrencySalt();   
       ...
       if($credit > 0 ){
        ...
        $calculatedAmount_INR1 = round($convertedAmount,2);  
        $calculatedAmount_INR = $calculatedAmount_INR1 - $credit; 
         } else{
                $calculatedAmount_INR = round($convertedAmount,2); 
        }
       } else {
          ...
          $calculatedAmount_INR1 = $this->getTotalAmount( $order );  
        $calculatedAmount_INR = $calculatedAmount_INR1 - $credit; 
         } else{
                  $calculatedAmount_INR = $this->getTotalAmount( $order ); 
        }
...
'offer_key' => 'DEBIT@7265'            
            ...
            if($payment_method_code == 'payucheckout_payumoney')
            {
              $data['pg'] = 'wallet';
              $data['bankcode'] = 'payuw';
            } else {  
            ...
            if ($payment_method_code == 'emi') {
              $data['enforce_paymethod'] = 'emi'; 
            }else if($payment_method_code == 'wallet'){
              $data['enforce_paymethod'] = 'cashcard'; 
        ...
        if(isset($response['status']))
        {
            ...
            if($response['status']=='success' || $response['status']=='in progress')
            {
                ...
                $responseHashString=$_REQUEST['hash'];
                  Mage::log($order->getIncrementId(), null, 'payu.log');
                ...
                if($sentHashString==$responseHashString && $order->getStatus() != 'payment_processing')
                {
                  ...
                  // Change to processing status for omni

                    foreach ($order->getAllItems() as $item) {

                        $query             = "UPDATE sales_flat_order_item SET item_state = 'processing', item_status = 'processing' WHERE order_id = '" . $order->getEntityId() . "' AND item_id = '" . $item->getItemId() . "'";
                        Mage::log($query, null, 'cancel.log');
    ...
    public function getCurrencyMerchantKey(){
      $currency_code = 'INR';//Mage::app()->getStore()->getCurrentCurrencyCode();
      $merchantKeys = Mage::getStoreConfig('payment/payucheckout_shared/merchant_key_salt');
      if ($merchantKeys) {
        $merchantKeys = unserialize($merchantKeys);
        if (is_array($merchantKeys)) {
            foreach($merchantKeys as $merchantKeysRow) {
              if ($merchantKeysRow['currency'] == $currency_code) {
                return $merchantKeysRow['mid'];
                break;
              }
    ...
    public function getCurrencySalt(){
      $currency_code = 'INR';//Mage::app()->getStore()->getCurrentCurrencyCode();
      $salts = Mage::getStoreConfig('payment/payucheckout_shared/merchant_key_salt');
      if ($salts) {
        $salts = unserialize($salts);
        if (is_array($salts)) {
            foreach($salts as $saltKeysRow) {
              if ($saltKeysRow['currency'] == $currency_code) {
                return $saltKeysRow['salt'];
                break;
              }
